...
func TestColumnPagination(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		models = append(models, model{
			ID:   (*paginate2.BigInt)(big.NewInt(int64(i))),
			Pair: i%2 == 0,
		})
	}
	...
	type testCase struct {
		name                  string
		query                 paginate2.ColumnPaginatedQuery[bool]
		expectedNext          *paginate2.ColumnPaginatedQuery[bool]
		expectedPrevious      *paginate2.ColumnPaginatedQuery[bool]
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			cursor, err := paginate2.UsingColumn[bool, model](context.Background(), query, tc.query)
			require.NoError(t, err)
			...
			if tc.expectedNext == nil {
				...
				q := paginate2.ColumnPaginatedQuery[bool]{}
