...
select (computed_move.asset,
        (computed_move.post_commit_aggregated_input, computed_move.post_commit_aggregated_output))
...
as $$
(
    select v.asset, (v.post_commit_aggregated_input, v.post_commit_aggregated_output)::volumes
    from get_latest_move_for_account_and_asset(_account, _asset, _before) v
    where not v.stale
) union all (
    select *
    from refresh_volumes(_account, _asset, _before) v
)
limit 1
$$;
...
with
    ...
    fresh_moves as (
        select moves.asset, (moves.post_commit_aggregated_input, moves.post_commit_aggregated_output)::volumes
        from moves
        where not moves.stale
    ),
    refreshed_moves as (
        select refreshed_move.asset, (refreshed_move.post_commit_aggregated_input, refreshed_move.post_commit_aggregated_output)::volumes
...
with
    ...
    fresh_moves as (
        select moves.asset, (moves.post_commit_aggregated_input, moves.post_commit_aggregated_output)::volumes as volumes
        from moves
        where not moves.stale
    ),
    refreshed_moves as (
        select refreshed_move.asset, (refreshed_move.post_commit_aggregated_input, refreshed_move.post_commit_aggregated_output)::volumes as volumes
        from moves, compute_move(moves) as refreshed_move
        where moves.stale
    )
select v.asset, (sum((v.volumes).inputs), sum((v.volumes).outputs))
from (
         select *
         from fresh_moves
         union all
...
from (
         select distinct on (safe_move.account_address, safe_move.asset) safe_move.account_address,
                                                                         volumes_to_jsonb((safe_move.asset, (first(safe_move.post_commit_aggregated_input), first(safe_move.post_commit_aggregated_output))::volumes)) as aggregated
         from moves move
